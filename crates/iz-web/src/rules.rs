//! Mail rules, ported from `iz-web/src/rules.rs` onto topcoat's
...
/// verbatim from `current_rules`'s match in `iz-web/src/rules.rs`.
fn sentence_of(
    trigger: &Trigger,
    columns: &[Column],
    lang: Lang,
        ...
        .map(|rule| {
            ...
            RuleLine {
                ...
                last_sent: last_sent
                    ...
                    .map(|(_, at)| iz_core::detail::moment_label_in(*at, zone)),
...
async fn create_rule(cx: &Cx, Form(input): Form<CreateRuleForm>) -> Redirect {
    ...
    let _ = store
        .record_event(
            Some(&user.id),
            &iz_core::detail::ActivityKind::RuleCreated,
            &subject,
            time::OffsetDateTime::now_utc(),
        )
        .await;
...
async fn update_rule(cx: &Cx, Form(input): Form<UpdateRuleForm>) -> Redirect {
    ...
    let _ = store
        .record_event(
            Some(&user.id),
            &iz_core::detail::ActivityKind::RuleEdited,
            &subject,
            time::OffsetDateTime::now_utc(),
        )
        .await;
...
async fn set_rule_enabled(cx: &Cx, Form(input): Form<SetRuleEnabledForm>) -> Redirect {
    ...
    let _ = store
        .record_event(
            Some(&user.id),
            &iz_core::detail::ActivityKind::RuleToggled,
            &rule.subject,
            time::OffsetDateTime::now_utc(),
        )
        .await;
...
async fn delete_rule(cx: &Cx, Form(input): Form<DeleteRuleForm>) -> Redirect {
    ...
    let _ = store
        .record_event(
            Some(&user.id),
            &iz_core::detail::ActivityKind::RuleDeleted,
            &rule.subject,
            time::OffsetDateTime::now_utc(),
        )
        .await;
...
/// `iz-web/src/rules.rs` used to hide it live — here it is decided once,
