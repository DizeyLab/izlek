...
impl Mail {
    ...
    pub async fn test(&self, to: &str) -> Option<Result<time::Duration, iz_core::MailError>> {
        let engine = self.0.clone()?;
        Some(engine.send_test(to).await)
    }

    /// Asks the mail server whether it would have us, sending nothing.
    pub async fn check(&self) -> Option<Result<time::Duration, iz_core::MailError>> {
        let engine = self.0.clone()?;
        Some(engine.check_sender().await)
    }

    fn log(report: iz_core::store::Result<iz_core::mail::Report>) {
        match report {
            Ok(report) if report.sent + report.failed + report.abandoned > 0 => {
                println!(
                    "iz mail  {} sent, {} to retry, {} given up on",
                    report.sent, report.failed, report.abandoned
                );
            }
            Ok(_) => {}
            Err(problem) => eprintln!("iz mail  the ledger could not be read: {problem}"),
        }
    }
}
...
pub fn stylesheet_guard(bundle: &AssetBundle) -> Result<String, String> {
    let expected = env!("IZ_STYLE_FINGERPRINT");
...
mod refusal_redirect_tests {
    ...
    fn a_bare_address_gains_a_query() {
        assert_eq!(
            carrying("http://iz.sh/", "cycle", "link_tasks").as_deref(),
    ...
    fn an_open_modal_stays_open() {
        assert_eq!(
            carrying("http://iz.sh/?task=DZ-01", "cycle", "link_tasks").as_deref(),
    ...
    fn a_second_refusal_replaces_the_first() {
        assert_eq!(
            carrying(
                "http://iz.sh/?task=DZ-01&refusal=cycle&on=link_tasks",
                "not-found",
                "link_tasks"
            )
            .as_deref(),
    ...
    fn a_call_with_no_name_carries_nothing() {
        assert_eq!(carrying("http://iz.sh/", "cycle", ""), None);
    }
}
