...
//!
//! Ported from `iz-web/src/auth.rs`. Every mutating call answers a browser
...
pub const WORKSPACE_NAME: &str = "İz";
...
async fn sign_in(cx: &Cx, Form(input): Form<SignInForm>) -> Redirect {
        ...
        .await
    {
        ...
        Err(error) => {
            if !matches!(error, iz_core::accounts::AccountError::RateLimited) {
                let _ = accounts(cx)
                    .store()
                    .record_event(
                        None,
...
/// `resend_link` (`iz-web/src/settings.rs`) — it lands back on Settings
...
pub async fn reset_by_token(cx: &Cx, token: &str) -> Result<Option<Invited>> {
    ...
    if link.kind != iz_core::store::LinkKind::Reset
