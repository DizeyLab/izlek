...
/// read, not explained.
pub(crate) fn outcome_word(outcome: iz_core::store::MailOutcome, lang: Lang) -> &'static str {
    use iz_core::store::MailOutcome;
    match outcome {
...
pub(crate) fn activity_sentence(
    ...
    match kind {
        ...
        ActivityKind::ClockSet => crate::i18n::clock_set_label(lang, detail),
        ActivityKind::ClockCleared => t(lang, Key::ActClockCleared).to_string(),
...
pub(crate) async fn event_happened(
    ...
    event_id: &str,
    lang: Lang,
) -> std::result::Result<Option<String>, iz_core::store::StoreError> {
    use iz_core::store::Event;
...
pub(crate) async fn decision_detail(
    ...
    board_id: Option<&str>,
    outcome: iz_core::store::MailOutcome,
    detail: &str,
    lang: Lang,
) -> std::result::Result<String, iz_core::store::StoreError> {
    use iz_core::store::MailOutcome;
...
/// The active section's own page size: from `iz_rows_<section>`, the
...
fn resolve_limit(cx: &Cx, section: Section) -> u32 {
    crate::server::presented_cookie(cx, &format!("iz_rows_{}", section_slug(section)))
            ...
            next_attempt: send.next_attempt_at.map(|at| {
                if at <= now {
                    ...
                    format!(
                        "{} {}",
                        t(lang, Key::NextTry),
                        iz_core::detail::moment_label_with_seconds_in(at, zone)
                    )
                }
            }),
        });
...
async fn retry_send(cx: &Cx, Form(input): Form<RetrySendForm>) -> Redirect {
    ...
    let _ = store
        .record_event(
            Some(&admin.id),
            &iz_core::detail::ActivityKind::Other("send_retried".to_string()),
            "",
...
async fn logs_page(cx: &Cx) -> Result {
    ...
    let zone = match require_admin(cx).await {
        Ok(user) => iz_core::detail::parse_zone(&user.timezone),
