...
pub struct DetailSnapshot {
    ...
    pub tags: Vec<iz_core::store::Tag>,
}
...
async fn set_task_tag(cx: &Cx, Form(input): Form<SetTaskTagForm>) -> Redirect {
    ...
    let _ = store
        .record_activity(
            ...
            Some(&actor.id),
            None,
            &iz_core::detail::ActivityKind::Other("tagged".to_string()),
            &tag.name,
...
async fn delete_file(cx: &Cx, Form(input): Form<FileIdForm>) -> Redirect {
    ...
    let _ = store
        .record_activity(
            ...
            Some(&user.id),
            None,
            &iz_core::detail::ActivityKind::FileRemoved,
...
pub(crate) async fn escape_closes(cx: &Cx) -> Result {
    ...
    const JS: &str = "\
        (function () { \
        if (window.__izEscModal) { return; } \
        window.__izEscModal = true; \
        ...
        document.addEventListener('click', function (e) { var confirm = document.querySelector('details.confirm-details[open]'); if (!confirm) { return; } var panel = confirm.querySelector('.confirm'); if (panel && !panel.contains(e.target) && !e.target.closest('summary')) { if (window.location.search.indexOf('confirm=delete') !== -1) { window.__izCloseModal(); } else { confirm.removeAttribute('open'); } } }, true); \
...
/// (`__izOwn(input, [], ['value'])`, and the panel's month attrs when
/// `render` first sets them), and an `iz:wire` repair pass re-derives the
...
pub(crate) async fn datepicker_script(cx: &Cx, lang: Lang) -> Result {
    ...
    let js = format!(
        "(function() {{\
            if (window.__izDatepick) {{ return; }}\
            window.__izDatepick = true;\
            var MONTHS = {months};\
            ...
            function render(panel) {{\
                ...
                var t = todayYmd();\
                if (!panel.dataset.year) {{ var base = sel || t; panel.dataset.year = base.y; panel.dataset.month = base.m; window.__izOwn(panel, [], ['data-year', 'data-month']); }}\
            ...
            document.addEventListener('iz:wire', function () {{\
                document.querySelectorAll('.datepick-pop').forEach(function (pop) {{\
                    ...
                    var input = panel.querySelector('.datepick-input');\
                    if (!input || !input.__izMine || input.__izMine.a.indexOf('value') === -1) {{ return; }}\
                    var label = pop.querySelector('.datepick-label');\
...
fn attachment_stamp(attachment: &iz_core::store::Attachment) -> String {
                        ...
                        <div class="activity-list">
                            for entry in &detail.activity {
                                <div class="activity-line">
                                    ...
                                    <strong class="activity-who">(entry.actor.as_ref().map(|person| person.display_name.clone()).unwrap_or_else(|| "İz".to_string()))</strong>
...
async fn audio_player_script(cx: &Cx) -> Result {
    ...
    const JS: &str = "\
        (function () { \
            if (window.__izAudioWired) { return; } \
            window.__izAudioWired = true; \
            ...
            window.__izOwn(player, [], ['data-wired']); \
            ...
            audio.addEventListener('play', function () { \
                window.__izOwn(player, ['audio-playing'], []); \
