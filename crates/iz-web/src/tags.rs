//! Task tags, ported from `iz-web/src/rules.rs` onto topcoat's
...
async fn tag_event(store: &Arc<dyn Store>, actor: &User, kind: &str, detail: &str) {
    let _ = store
        .record_event(
            Some(&actor.id),
            &iz_core::detail::ActivityKind::Other(kind.to_string()),
            detail,
...
async fn delete_tag(cx: &Cx, Form(input): Form<DeleteTagForm>) -> Redirect {
    ...
    match store.delete_tag(&input.tag_id).await {
        ...
        Err(iz_core::store::StoreError::Conflict("tag_in_use")) => {
