...
//!
//! Ported from the Leptos-era `iz-web`'s `tests/http.rs`. That app answered
...
impl App {
    ...
    /// naming — for the `iz_rows_<section>` page-size cookie, which lives
    ...
    async fn post_multipart(
        ...
        const BOUNDARY: &str = "iz-test-boundary";
    ...
    /// naming — for the `iz_rows_<section>` page-size cookie, which
...
async fn a_viewer_who_posts_to_create_task_anyway_is_refused() {
    ...
    let viewer = invited(&app, &admin, "quiet@iz.sh", "Quiet Reader", Role::Viewer).await;
...
async fn a_member_may_create_a_task() {
    let app = App::open().await;
    let admin = admin(&app).await;
    let member = invited(&app, &admin, "mo@iz.sh", "Mo Dubois", Role::Member).await;
    let column = first_column(&app).await;
...
async fn the_board_sort_control_keeps_its_hidden_select_and_gets_a_dropdown_trigger() {
    ...
    assert!(
        html.contains("window.__izEsc"),
        "no escape manager on the board page: {html}"
    );
    assert!(
        html.contains("__izEsc.register(40"),
...
async fn a_viewer_who_posts_a_comment_anyway_is_refused() {
    let app = App::open().await;
    let admin = admin(&app).await;
    let viewer = invited(&app, &admin, "eyes@iz.sh", "Ida Eyes", Role::Viewer).await;
    let column = first_column(&app).await;
...
async fn a_member_may_comment_and_the_author_is_the_session() {
    let app = App::open().await;
    let admin = admin(&app).await;
    let member = invited(&app, &admin, "kai@iz.sh", "Kai Renner", Role::Member).await;
    let column = first_column(&app).await;
...
async fn a_refusal_reaches_a_browser_with_no_script() {
    ...
    let answer = app
        .post_without_script(
            "/api/link_tasks",
            Some(&admin),
            &format!("http://iz.test/?task={first}"),
            &[
                ("task_id", &first),
                ("other_id", &second),
                ("direction", "blocked_by"),
...
async fn a_viewer_who_posts_a_delete_anyway_is_refused() {
    let app = App::open().await;
    let admin = admin(&app).await;
    let viewer = invited(&app, &admin, "wren@iz.sh", "Wren Ash", Role::Viewer).await;
    let column = first_column(&app).await;
...
async fn a_member_may_delete_and_the_delete_is_soft() {
    let app = App::open().await;
    let admin = admin(&app).await;
    let member = invited(&app, &admin, "rae@iz.sh", "Rae Okonkwo", Role::Member).await;
    let column = first_column(&app).await;
...
async fn a_viewer_may_not_open_or_move_a_subtask() {
    let app = App::open().await;
    let admin = admin(&app).await;
    let watcher = invited(&app, &admin, "kay@iz.sh", "Kay Watcher", Role::Viewer).await;
    let columns = columns_of(&app).await;
    let parent = a_task(&app, &admin, &columns[0], "Ship the exporter").await;
...
async fn the_morph_keeps_what_the_client_owns_and_names_none_of_it() {
    ...
    assert!(
        page.contains("window.__izOwn = function (node, classes, attrs)"),
        "there is no way left for an enhancement to declare what it owns"
    );
    assert!(
        page.contains("window.__izAdded = function (node)"),
    ...
    assert!(
        page.contains("function clientMade(node) { return node.__izAdded === true; }"),
        "the stray test is back to sniffing class names"
    );
    assert!(
        page.contains("var own = from.__izMine"),
    ...
    assert!(
        page.contains("window.__izOwn(select, ['dd-native'], [])"),
...
async fn the_rules_and_logs_pages_get_the_shared_escape_script() {
    ...
    assert!(
        rules_html.contains("__izEsc.register(40"),
    ...
    assert!(
        logs_html.contains("__izEsc.register(40"),
...
async fn a_sender_field_that_cannot_work_is_refused_by_name() {
    ...
    let bad: &[(&str, &[(&str, &str)], &str)] = &[
        ...
        (
            "from_address",
            &[("from_address", "board-at-iz")],
            "That is not a from-address.",
        ),
        (
            "from_address",
            &[("from_address", "board@iz")],
            "That is not a from-address.",
        ),
    ];
...
async fn a_viewer_may_not_press_the_test_button() {
    let app = App::open().await;
    let admin_cookie = admin(&app).await;
    let viewer = invited(&app, &admin_cookie, "pinar@iz.sh", "Pinar", Role::Viewer).await;
...
async fn an_admin_sees_the_sender_and_never_a_password() {
    ...
    assert!(html.contains("iz@iz.sh"), "{html}");
...
async fn a_member_is_not_sent_the_member_list() {
    ...
    let admin_cookie = admin(&app).await;
    let member = invited(&app, &admin_cookie, "emre@iz.sh", "Emre", Role::Member).await;
    let _ = invited(&app, &admin_cookie, "quiet@iz.sh", "Quiet", Role::Viewer).await;

    let page = app.get("/settings", Some(&member)).await;
    let html = String::from_utf8_lossy(&page.bytes);
    assert!(!html.contains("member-table"), "{html}");
    assert!(!html.contains("quiet@iz.sh"), "{html}");
}
...
async fn the_viewer_renders_in_page_for_a_renderable_file_and_ignores_a_foreign_or_missing_one() {
    ...
    assert!(
        html.contains("__izEsc.register(90"),
...
async fn only_the_uploader_or_an_admin_may_delete_a_file() {
    let app = App::open().await;
    let admin_cookie = admin(&app).await;
    let member_a = invited(&app, &admin_cookie, "asha@iz.sh", "Asha", Role::Member).await;
    let member_b = invited(&app, &admin_cookie, "beau@iz.sh", "Beau", Role::Member).await;
    let column = first_column(&app).await;
...
async fn an_admin_reads_the_logs() {
    ...
    let admin_cookie = admin(&app).await;
    let mate = invited(&app, &admin_cookie, "emre@iz.sh", "Emre", Role::Member).await;
    let columns = columns_of(&app).await;
...
async fn a_rule_rides_every_event_and_can_be_rewritten() {
    ...
    let decisions = app
        .store
        .recent_mail_decisions(50, iz_core::store::FeedPage::Newest)
        .await
        .unwrap();
    assert!(
        decisions.iter().any(|decision| decision.rule_id == rule
            && matches!(decision.outcome, iz_core::store::MailOutcome::Owed)),
    ...
    until_rule_send_to(&app, &rule, "ada@iz.sh", already).await;
    assert!(
        app.store
            .mail_queue(50, iz_core::store::FeedPage::Newest)
            .await
    ...
    assert!(logs.contains("\"recipient\":\"ada@iz.sh\""), "{}", logs);
}
...
async fn the_activity_filter_narrows_and_the_position_note_counts_the_match() {
    ...
    let admin_id = user_id(&app, "ada@iz.sh").await;
    let member = invited(&app, &admin_cookie, "deniz@iz.sh", "Deniz", Role::Member).await;
    let member_id = user_id(&app, "deniz@iz.sh").await;
...
async fn the_kind_filter_shows_only_that_kinds_rows() {
    ...
    for i in 0..5 {
        app.store
            .record_event(
                None,
                &iz_core::detail::ActivityKind::Created,
                "",
                t0 + time::Duration::seconds(i),
            )
            .await
    ...
    for i in 0..3 {
        app.store
            .record_event(
                None,
                &iz_core::detail::ActivityKind::Commented,
                "",
...
/// The `iz_rows_activity` cookie the page's own fit script sets caps the
...
async fn the_older_href_round_trips_the_active_filter() {
    ...
    for i in 0..60 {
        app.store
            .record_event(
                None,
                &iz_core::detail::ActivityKind::Other("target".to_string()),
                &format!("target {i}"),
                t0 + time::Duration::seconds(i),
            )
            .await
    ...
    for i in 0..5 {
        app.store
            .record_event(
                None,
                &iz_core::detail::ActivityKind::Other("foreign".to_string()),
                &format!("foreign {i}"),
...
/// small `iz_rows_activity` cookie — the JSON route never widened by the
...
async fn a_tasks_notifications_show_the_recipient_and_the_rule_name_is_admin_only() {
    ...
    assert!(member_html.contains("ada@iz.sh"), "{member_html}");
    assert!(
    ...
    assert!(admin_html.contains("ada@iz.sh"), "{admin_html}");
...
async fn the_mail_servers_own_words_are_admin_only() {
    ...
    assert!(
        member_html.contains("ada@iz.sh"),
...
async fn an_admin_retries_a_failed_send_and_it_rejoins_the_queue() {
    ...
    let logs = app
        ...
        .await;
    assert!(
        logs.body.contains("\"recipient\":\"ada@iz.sh\""),
...
async fn an_admin_sends_a_message_to_one_member() {
    ...
    let mert_id = app
        .store
        .user_by_email(&app.workspace_id().await, "emre@iz.sh")
        .await
        .unwrap()
        .unwrap()
        .id;
    ...
    assert_eq!(notice.recipient, "emre@iz.sh");
...
async fn an_admin_sends_a_message_to_everyone_and_not_to_themself() {
    ...
    assert!(
        notices.iter().all(|send| send.recipient != "ada@iz.sh"),
        "{sends:?}"
    );
}
...
async fn the_live_channel_never_names_an_admin_surface_to_a_member() {
    ...
    let member = invited(&app, &boss, "bo@iz.sh", "Bo", Role::Member).await;
    ...
    app.store
        .create_mail_rule(
            &board,
            &iz_core::store::Trigger::Created,
            "Something happened",
            iz_core::store::Audience::Board,
            time::OffsetDateTime::now_utc(),
            false,
        )
        .await
...
async fn a_tag(app: &App, cookie: &str, name: &str) -> iz_core::store::Tag {
    let answer = app
...
async fn a_non_admin_cannot_create_rename_delete_or_move_a_tag() {
    let app = App::open().await;
    let admin_cookie = admin(&app).await;
    let member = invited(&app, &admin_cookie, "mem@iz.sh", "Mem Ber", Role::Member).await;
...
async fn a_task_by(
    ...
    app.store
        .create_task(iz_core::store::NewTask {
            board_id,
            column_id,
            parent_id: None,
            title,
...
async fn a_time_set_on_an_existing_day_writes_both_columns() {
    ...
    let saved = app
        .post(
            "/api/save_task",
            Some(&admin_cookie),
            &[("task_id", &task), ("deadline", "2027-09-02")],
        )
        .await;
    assert!(
        !saved
...
async fn the_board_assignee_filter_narrows_and_a_bogus_id_falls_back_to_all() {
    let app = App::open().await;
    let admin_cookie = admin(&app).await;
    invited(&app, &admin_cookie, "deniz@iz.sh", "Deniz", Role::Member).await;
    let columns = columns_of(&app).await;
...
fn the_stylesheet_guard_refuses_a_foreign_bundle() {
    ...
    let problem = iz_web::server::stylesheet_guard(&foreign_bundle)
