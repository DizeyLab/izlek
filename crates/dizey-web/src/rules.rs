...
#[cfg(feature = "ssr")]
fn audience_word(audience: dizey_core::store::Audience) -> &'static str {
    use dizey_core::store::Audience;
    match audience {
        ...
        .map(|rule| {
            ...
            RuleLine {
                ...
                last_sent: last_sent
                    ...
                    .map(|(_, at)| dizey_core::detail::moment_label(*at)),
                id: rule.id,
            }
        })
        .collect();
...
pub async fn create_rule(
    ...
    use crate::server::{accounts, require_admin};
    use dizey_core::store::{Audience, Trigger};
...
async fn rule_of_this_workspace(
    rule_id: &str,
) -> Result<std::sync::Arc<dyn dizey_core::store::Store>, Refusal> {
    use crate::server::{accounts, require_admin};
