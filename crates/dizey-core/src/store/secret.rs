...
//! file (`dizey.key`, beside the database) that a database backup does not
...
mod tests {
    ...
    fn load_or_create_key_persists_and_restricts() {
        let dir = std::env::temp_dir().join(format!("dizey-key-test-{}", uuid_lite()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("dizey.key");
