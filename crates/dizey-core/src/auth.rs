...
mod tests {
    ...
    fn known_answer_for_the_token_digest() {
        // SHA-256("dizey"), so a refactor cannot quietly change the algorithm
        // under stored hashes.
        assert_eq!(
            hash_token("dizey"),
            "ecef066ddcba712d09eca07be6a8625343e795c97b73a7f2f03417cce1fa58d7"
        );
    }
    ...
    fn password_rules_are_the_ones_on_the_screen() {
        ...
        assert_eq!(
            check_password("grace@dizey.sh!!", email, name),
            Err(PasswordProblem::LooksLikeYou)
        );
...
mod dummy_hash_generator {
    //! Run with `cargo test -p dizey-core --lib print_a_dummy_hash -- --ignored
