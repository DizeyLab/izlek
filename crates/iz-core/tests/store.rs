...
async fn raw_conn(scratch: &Scratch) -> turso::Connection {
    let db = turso::Builder::new_local(scratch.dir.join("iz.db").to_str().unwrap())
        .build()
        .await
        .unwrap();
    db.connect().unwrap()
...
async fn a_parent_does_not_finish_before_its_subtasks_do() {
    ...
    assert!(
        !detail
            .activity
            .iter()
            .any(|entry| matches!(entry.kind, iz_core::detail::ActivityKind::Moved)),
...
async fn the_board_counts_subtasks_instead_of_carding_them() {
    ...
    let cards: Vec<&iz_core::board::TaskCard> =
...
/// A backup restored without its sibling `iz.key`, or a key file damaged
...
async fn the_database_and_key_file_are_owner_only() {
    ...
    let scratch = Scratch::open().await;
    let db_path = scratch.dir.join("iz.db");
    let key_path = scratch.dir.join("iz.key");
...
impl BoardReads for CountingReads<'_> {
    async fn board(
        &self,
        workspace_id: &str,
    ) -> Result<Option<iz_core::board::BoardMeta>, StoreError> {
        self.tick();
        self.inner.board(workspace_id).await
    }

    async fn columns(&self, board_id: &str) -> Result<Vec<iz_core::board::Column>, StoreError> {
        self.tick();
    ...
    async fn tasks_for_board(
        &self,
        board_id: &str,
    ) -> Result<Vec<iz_core::board::TaskRow>, StoreError> {
        self.tick();
...
async fn count_and_filtered_keyset_walk_cover_exactly_the_filtered_set() {
    use iz_core::store::{ActivityFilter, Dir, FeedCursor, FeedPage};

    let (scratch, workspace, admin) = workspace_with_admin().await;
    let store = &scratch.store;
    let other = member(store, &workspace, "sam@iz.sh", "Sam").await;
    ...
    for i in 0..20 {
        ...
        store
            .record_event(
                Some(actor),
                &iz_core::detail::ActivityKind::Other("row".to_string()),
                &format!("row {i}"),
...
fn tag_order(tags: &[iz_core::store::Tag]) -> Vec<&str> {
...
impl DetailReads for CountingDetail<'_> {
    ...
    async fn files_for_task(
        &self,
        task_id: &str,
    ) -> Result<Vec<iz_core::detail::FileLine>, StoreError> {
        self.tick();
    ...
    async fn family_for_task(
        &self,
        task_id: &str,
    ) -> Result<Vec<(bool, iz_core::detail::SubtaskLine)>, StoreError> {
        self.tick();
    ...
    let refused = conn
        .execute(
            "INSERT INTO user (id, workspace_id, email, display_name, role, created_at) \
             VALUES ('u-orphan', 'no-such-workspace', 'orphan@iz.sh', 'Orphan', \
             'member', '2026-08-26')",
            (),
        )
        .await;
...
async fn the_index_decides_who_owns_a_send() {
    ...
    let first = scratch
        .store
        .claim_send(&rule.id, &transition.id, &task, "ada@iz.sh", now, now)
        .await
        .unwrap();
...
async fn a_refused_address_is_not_retried_forever() {
    ...
    let send = scratch
        .store
        .claim_send(&rule.id, &transition.id, &task, "gone@iz.sh", now, now)
        .await
        .unwrap()
        .unwrap();
...
async fn a_viewer_is_never_a_recipient() {
    ...
    for list in [
        ...
        assert!(
            list.iter().all(|person| person.email != "viewer@iz.sh"),
...
use iz_core::mail::{Engine, MailError, Mailer, Outgoing, Report, backoff};
use std::sync::Mutex;
...
async fn a_crossing_mails_the_people_on_the_card_once() {
    ...
    assert!(
        sent[0].body.contains("https://iz.sh/?task="),
...
async fn a_failing_send_is_retried_a_bounded_number_of_times_and_then_written_off() {
    ...
    assert_eq!(ledger.attempts, iz_core::mail::MAX_ATTEMPTS);
...
async fn a_file_name_that_is_a_path_is_stored_as_the_label_it_is() {
    ...
    let written: Vec<String> = std::fs::read_dir(&scratch.dir)
        .unwrap()
        .map(|entry| entry.unwrap().file_name().to_string_lossy().into_owned())
        .filter(|name| !name.starts_with("iz.db") && name != "iz.key" && name != "storage")
        .collect();
...
async fn keyset_paging_covers_the_queue_ascending() {
    use iz_core::store::{FeedCursor, FeedPage};
...
async fn the_queue_shows_what_is_owed_and_not_what_is_done() {
    ...
    let failed = store
        .claim_send(&rule.id, &transition.id, &task, "failed@iz.sh", now, now)
        .await
        .unwrap()
        .unwrap();
    store
    ...
    let sent = store
        .claim_send(&rule.id, &transition.id, &task, "sent@iz.sh", now, now)
        .await
        .unwrap()
        .unwrap();
...
async fn an_invite_mail_with_no_sender_is_held_not_failed() {
    ...
    let held = queue
        .iter()
        .find(|s| s.recipient == "newcomer@iz.sh")
        .unwrap();
...
async fn two_passes_over_one_owed_mail_send_it_once() {
    ...
    let request = Engine::new(store.clone(), mailer.clone(), "https://iz.sh");
    let sweep = Engine::new(store.clone(), mailer.clone(), "https://iz.sh");
    ...
    // mail being dropped.
    let owed = store
        .sends_owed(now + iz_core::mail::LEASE + Duration::seconds(1), 10)
        .await
        .unwrap();
    assert!(
        owed.is_empty(),
...
async fn a_crossing_and_a_sweep_at_the_same_moment_mail_once() {
    ...
    let crossing = Engine::new(store.clone(), mailer.clone(), "https://iz.sh");
    let sweep = Engine::new(store.clone(), mailer.clone(), "https://iz.sh");
...
async fn a_lease_that_expires_gives_the_mail_back() {
    ...
    let mine = store
        .claim_sends_owed(now, now + iz_core::mail::LEASE, 10)
        .await
        .unwrap();
    assert!(
...
async fn activity_event(store: &TursoStore, activity_id: &str) -> iz_core::store::ActivityEvent {
...
async fn assigning_one_person_mails_that_person_not_every_assignee() {
    let (dir, store, workspace, admin) = shared().await;
    let first = member(&store, &workspace, "grace@iz.sh", "Grace").await;
    let second = member(&store, &workspace, "emre@iz.sh", "Emre").await;
    let third = member(&store, &workspace, "linus@iz.sh", "Linus").await;
    let task = add_task(&store, &workspace, "Backlog", "Ship it", None, &admin).await;
    ...
    let fourth = member(&store, &workspace, "yusuf@iz.sh", "Yusuf").await;
    let activity_id = store
        .record_activity(
            &task,
            Some(&admin),
...
async fn an_unassignment_mails_the_person_removed_not_the_rest() {
    let (dir, store, workspace, admin) = shared().await;
    let staying = member(&store, &workspace, "grace@iz.sh", "Grace").await;
    let leaving = member(&store, &workspace, "emre@iz.sh", "Emre").await;
    let task = add_task(&store, &workspace, "Backlog", "Ship it", None, &admin).await;
...
async fn a_rule_whose_line_has_no_subject_still_mails_every_assignee() {
    ...
    to.sort();
    assert_eq!(to, vec!["emre@iz.sh", "grace@iz.sh"]);
    let _ = std::fs::remove_dir_all(&dir);
}
...
async fn a_queued_notice_is_pending_and_due_and_on_the_queue() {
    ...
    let queued = scratch
        .store
        .queue_notice("grace@iz.sh", "Heads up", "Body text", now)
        .await
        .unwrap();
    assert_eq!(queued.kind, SendKind::Notice);
    assert_eq!(queued.state, SendState::Pending);
    assert_eq!(queued.recipient, "grace@iz.sh");
...
async fn the_engine_delivers_a_notice_that_owes_no_rule() {
    ...
    assert_eq!(sent[0].to, "grace@iz.sh");
...
fn announced(rx: &mut tokio::sync::broadcast::Receiver<iz_core::Change>) -> Vec<String> {
...
async fn every_kind_of_write_announces_its_surface() {
    ...
    store
        ...
        .unwrap();
    // Members.
    member(store, &workspace, "bo@iz.sh", "Bo").await;
    // Queue.
    store
        .queue_notice("bo@iz.sh", "Subject", "Body", now)
        .await
        .unwrap();
    // Rules.
    store
    ...
    // Settings.
    store
        .set_public_url(&workspace, Some("https://iz.example"))
        .await
        .unwrap();
    ...
    store
        .record_event(
            Some(&admin),
            &iz_core::detail::ActivityKind::Other("settings".to_string()),
            "",
            now,
        )
        .await
...
async fn the_next_due_moment_is_the_earliest_one() {
    ...
    store
        .queue_notice("late@iz.sh", "Later", "body", now + Duration::hours(2))
        .await
        .unwrap();
    store
...
// ---------------------------------------------------------------------------
// iz reconcile — bringing a database of the OLD shape onto the declared
...
async fn reconcile_carries_a_live_shaped_database_onto_the_declared_schema() {
    ...
    let after = iz_core::store::schema::fingerprint(&conn).await.unwrap();
    let declared = iz_core::store::schema::declared_fingerprint()
        .await
        .unwrap();
    assert_eq!(
        after, declared,
...
async fn reconciling_a_database_that_already_matches_changes_nothing() {
    let scratch = Scratch::open().await;
    let path = scratch.dir.join("iz.db").to_str().unwrap().to_string();
...
async fn opening_a_stale_database_repairs_it() {
    ...
    let conn = db.connect().unwrap();
    assert_eq!(
        iz_core::store::schema::fingerprint(&conn).await.unwrap(),
        iz_core::store::schema::declared_fingerprint()
            .await
            .unwrap(),
    );
    assert_eq!(scalar(&path, "SELECT COUNT(*) FROM task").await, 4);
...
async fn two_repairs_leave_two_backups() {
    let (dir, path) = live_shaped_database().await;
    let opts = || iz_core::store::ReconcileOptions {
        dry_run: false,
        yes: true,
        auto: true,
    };
...
async fn user_stats_counts_what_a_person_holds_finished_opened_and_said() {
    ...
    let (workspace, admin) = claim(&scratch.store).await;
    let store = &scratch.store;
    let mem = member(store, &workspace, "mem@iz.sh", "Mem Ber").await;
    let backlog = column_named(store, &workspace, "Backlog").await;
    let done = column_named(store, &workspace, "Done").await;
    let now = OffsetDateTime::now_utc();
...
async fn a_deleted_task_counts_nowhere_and_a_person_with_nothing_counts_nothing() {
    ...
    let store = &scratch.store;
    let mem = member(store, &workspace, "mem@iz.sh", "Mem Ber").await;
    let shy = member(store, &workspace, "shy@iz.sh", "Shy Guy").await;
    let now = OffsetDateTime::now_utc();
...
async fn the_one_row(store: &TursoStore) -> iz_core::store::MailSend {
    let mut rows = store
        .sends_owed(OffsetDateTime::now_utc() + Duration::hours(2), 10)
        .await
        .unwrap();
...
async fn the_hold_has_a_ceiling_measured_from_the_oldest_mail() {
    ...
    store
        .claim_send(&rule.id, "event-1", &task, "emre@iz.sh", born, born)
        .await
        .unwrap()
    ...
    store
        .hold_batch(&task, "emre@iz.sh", far, Duration::minutes(4))
        .await
        .unwrap();
...
async fn a_hold_never_pulls_a_leased_row_back_under_the_pass_that_took_it() {
    ...
    let now = OffsetDateTime::now_utc();
    store
        .claim_send(&rule.id, "event-1", &task, "emre@iz.sh", now, now)
        .await
        .unwrap()
        .expect("a fresh row is claimed");
...
async fn a_batch_is_one_card_and_one_reader() {
    ...
    assert_eq!(
        addressed
            .iter()
            .filter(|line| line.contains("emre@iz.sh"))
            .count(),
        2,
    );
    let _ = std::fs::remove_dir_all(&dir);
...
async fn a_clocked_task_owes_each_assignee_one_reminder_before_the_meeting() {
    ...
    assert_eq!(rows[1].recipient, "grace@iz.sh");
    let _ = std::fs::remove_dir_all(&dir);
}
...
async fn an_assignee_removed_takes_their_reminder_with_them() {
    ...
    assert!(
        rows.iter()
            .any(|r| r.recipient == "grace@iz.sh" && r.state == SendState::Abandoned),
    ...
    assert_eq!(still_owed[0].recipient, "emre@iz.sh");
    let _ = std::fs::remove_dir_all(&dir);
}
...
async fn a_reminder_already_sent_is_never_queued_again() {
    ...
    let yusuf = member(&store, &workspace, "yusuf@iz.sh", "Yusuf").await;
    store.assign_task(&task, &yusuf).await.unwrap();
    let rows = reminders(&store, &task).await;
    let yusuf_rows: Vec<_> = rows
        .iter()
        .filter(|r| r.recipient == "yusuf@iz.sh")
        .collect();
    ...
    for person in ["grace@iz.sh", "emre@iz.sh"] {
        assert_eq!(
            served
                .iter()
    ...
    assert_eq!(pending[0].recipient, "yusuf@iz.sh");
...
async fn the_reminder_never_lands_at_or_after_the_deadline() {
    ...
    store
        .assign_task(
            &task,
            &member(&store, &workspace, "grace@iz.sh", "Grace").await,
        )
        .await
        .unwrap();
    let rows = reminders(&store, &task).await;
    ...
    store
        .assign_task(
            &task,
            &member(&store, &workspace, "emre@iz.sh", "Emre").await,
        )
        .await
        .unwrap();
    let rows = reminders(&store, &task).await;
...
async fn board_search_matches_keys_and_titles_in_place() {
    ...
    let mut board = iz_core::board::load(&scratch.store, &workspace)
        .await
        .unwrap()
        .unwrap();
