...
mod tests {
    ...
    fn known_answer_for_the_token_digest() {
        // SHA-256("iz"), so a refactor cannot quietly change the algorithm
        // under stored hashes.
        assert_eq!(
            hash_token("iz"),
            "a93c5a18c1c1da1c799d05b4be13c8de48b5fb5db7d686c3356614cf21ae7855"
        );
    }
    ...
    fn password_rules_are_the_ones_on_the_screen() {
        ...
        assert_eq!(
            check_password("grace@iz.sh!!", email, name),
            Err(PasswordProblem::LooksLikeYou)
        );
...
mod dummy_hash_generator {
    //! Run with `cargo test -p iz-core --lib print_a_dummy_hash -- --ignored
