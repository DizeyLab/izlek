...
//! `crates/iz-core/migrations` at boot and records the version.
...
/// beside the main one (`iz.db-wal`, `iz.db-shm`).
...
fn load_key(path: &str) -> Result<secret::Key> {
    ...
    secret::load_or_create_key(&dir.join("iz.key")).map_err(backend)
}
...
impl Store for TursoStore {
    ...
    async fn smtp_password(&self, workspace_id: &str) -> Result<Option<String>> {
        ...
        // backup missing its `iz.key` — comes back as `None` rather than
    ...
    impl Fixture {
        async fn open() -> Self {
            ...
            let store = TursoStore::open(
                dir.path().join("iz.db").to_str().unwrap(),
